import 'dart:convert';
//import 'package:focus_chronicle/helpers/focus_localizations.dart';
...
import 'focus_localizations.dart';
...
class SearchDemoSearchDelegate extends SearchDelegate<int> {
  ...
  List<Widget> buildActions(BuildContext context) {
    return <Widget>[
      query.isEmpty
          ...
          : IconButton(
              tooltip:
                  DecoLocalizations.of(context).localizedString("search_clear"),
  ...
  //         : Colors.white,
  //     primaryIconTheme:
  //         theme.primaryIconTheme.copyWith(color: Color(0xFFb3bbbf)),
    ...
    if (searchResultsCount > 0) {
      ...
      _data = (searchResultsData as List).map((post) {
        CategoryModel category =
            categories.firstWhere((cat) => cat.id == post['categories'][0]);
  ...
  _presentPost(BuildContext context, PostModel post) {
    ...
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (context) => SinglePost(post)));
  }
}
