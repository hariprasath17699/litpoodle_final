...
class _SingleCategorySliderScreenState extends State<SingleCategorySliderScreen>
            ...
            children: <Widget>[
              ...
              if (featured.length > 0)
                Container(
                  ...
                  child: Carousel(
                    showIndicator: true,
                    //dotColor: Color(0xFFaf8521),
                    dotSelectedColor: secondarycolor,
                    // dotSelectedColor: Color(0xFFaf8521),
                    autoplay: true,
  ...
  _loadData({clear = false}) async {
    ...
    if (response.statusCode == 200) {
      int totalPosts = 0;
      if (response.headers['x-wp-total'] != null) {
        totalPosts = int.parse(response.headers['x-wp-total'].toString());
      }
      if (mounted) {
        setState(() {
